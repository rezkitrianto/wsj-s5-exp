...
label_delay=0
dropout_schedule='0,0@0.20,0.1@0.50,0@0.50,0@0.50,0'
...
dir=${srcpath}exp/chain/tdnn_blstm_1a_7layers_dropout1_sp_noIvector
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=blstm6-forward input=Append(blstm5-forward, blstm5-backward) cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 $lstm_opts
  fast-lstmp-layer name=blstm6-backward input=Append(blstm5-forward, blstm5-backward) cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=3 $lstm_opts

  fast-lstmp-layer name=blstm7-forward input=Append(blstm6-forward, blstm6-backward) cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 $lstm_opts
  fast-lstmp-layer name=blstm7-backward input=Append(blstm6-forward, blstm6-backward) cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=3 $lstm_opts

  ## adding the layers for chain branch
  output-layer name=output input=Append(blstm7-forward, blstm7-backward) output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5
  ...
  output-layer name=output-xent input=Append(blstm7-forward, blstm7-backward) output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5
