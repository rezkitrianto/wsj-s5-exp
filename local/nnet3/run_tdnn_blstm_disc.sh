...
destpath=/media/rezki/8A6CF1066CF0EE37/kaldi/exp/
...
nj=7 # have a high number of jobs because this could take a while, and we might
...
# gmm_dir=exp/tri4b
gmm_dir=$gmm_res
srcdir=${srcpath}exp/chain/tdnn_blstm_1a_7layers_sp
# train_data_dir=data/${train_set}_sp_hires
train_data_dir=$data_fmllr/tr05_multi_noisy
# online_ivector_dir=exp/nnet3/ivectors_${train_set}_sp_hires
online_ivector_dir=${srcpath}exp/nnet3/ivectors_tr05_multi_noisy_fmllr
# dir=${srcdir}_${criterion}${disc_affix}
dir=${destpath}chain/tdnn_blstm_1a_7layers_sp_${criterion}${disc_affix}
...
if [ $stage -le 1 ]; then
  ...
  steps/nnet3/align.sh  --cmd "$decode_cmd" --use-gpu false \
    ...
    --nj $nj $train_data_dir ${srcpath}data/lang $srcdir ${srcdir}_ali ;
fi
...
if [ -z "$degs_dir" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    steps/nnet3/get_degs.sh \
      ...
      $train_data_dir ${srcpath}data/lang ${srcdir} ${srcdir}_ali ${srcdir}_degs || exit 1
  fi
fi
