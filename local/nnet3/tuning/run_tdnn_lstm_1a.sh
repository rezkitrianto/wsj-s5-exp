...
dir=${destpath}exp/chain${nnet3_affix}/tdnn${affix}_sp
# dir=/home/rezki/sandbox/kaldi/egs/chime4DevFE1_allChannels3/s5_6ch/exp/chain/tdnn31a_sp_blstm
...
dropout_schedule='0,0@0.20,0.1@0.50,0@0.50,0@0.50,0@0.50,0@0.50,0'
...
if [ $stage -le 12 ]; then
  ...
  lstm_opts="dropout-proportion=0.0"
  ...
  relu-renorm-layer name=tdnn7 dim=520 input=Append(-3,0,3)
  relu-renorm-layer name=tdnn8 dim=520 input=Append(-3,0,3)
  fast-lstmp-layer name=lstm4 cell-dim=520 recurrent-projection-dim=130 non-recurrent-projection-dim=130 decay-time=20 delay=-3
  relu-renorm-layer name=tdnn9 dim=520 input=Append(-3,0,3)
  relu-renorm-layer name=tdnn10 dim=520 input=Append(-3,0,3)
  fast-lstmp-layer name=lstm5 cell-dim=520 recurrent-projection-dim=130 non-recurrent-projection-dim=130 decay-time=20 delay=-3
  relu-renorm-layer name=tdnn11 dim=520 input=Append(-3,0,3)
  relu-renorm-layer name=tdnn12 dim=520 input=Append(-3,0,3)
  fast-lstmp-layer name=lstm6 cell-dim=520 recurrent-projection-dim=130 non-recurrent-projection-dim=130 decay-time=20 delay=-3
  relu-renorm-layer name=tdnn13 dim=520 input=Append(-3,0,3)
  relu-renorm-layer name=tdnn14 dim=520 input=Append(-3,0,3)
  fast-lstmp-layer name=lstm7 cell-dim=520 recurrent-projection-dim=130 non-recurrent-projection-dim=130 decay-time=20 delay=-3

  output-layer name=output input=lstm7 output-delay=$label_delay dim=$num_targets max-change=1.5
    ...
    (
      ...
      for lmtype in graph_tgpr_5k; do
        ...
        steps/nnet3/decode.sh \
          ...
          $graph_dir3 ${data_fmllr}/${data} ${dir}/decode2_${lmtype}_${data_affix} || exit 1
      done
